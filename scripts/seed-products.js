...
// Not wired into the app or package.json scripts. Run directly:
...
import cloudinary from "../backend/lib/cloudinary.js";
import Product from "../backend/models/product.model.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
dotenv.config({ path: path.resolve(__dirname, "../.env") });

// category values must match the URL slugs in frontend/src/pages/HomePage.jsx's
...
async function seed() {
	...
	for (const item of PRODUCTS) {
		...
		const imagePath = path.resolve(__dirname, "../frontend/public", item.image);
