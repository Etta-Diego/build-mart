...
const AnalyticsTab = () => {
	...
	useEffect(() => {
		// Baseline has no Analytics service and no Gateway to compose one
		// call server-side (see docs/DECISION_LOG.md) - each service exposes
		// a small summary of its own data, and this component does the
		// composition a Gateway would otherwise do in Enhanced. Three
		// round trips instead of one is itself a measurable, citable
		// Baseline-vs-Enhanced contrast, not just an implementation detail.
		const fetchAnalyticsData = async () => {
			try {
				const [usersRes, productsRes, summaryRes] = await Promise.all([
					userApi.get("/auth/count"),
					productApi.get("/products/count"),
					orderApi.get("/orders/summary"),
				]);
				...
				setDailySalesData(summaryRes.data.dailySalesData);
			} catch (error) {
