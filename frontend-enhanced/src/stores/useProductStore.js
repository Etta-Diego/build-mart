...
export const useProductStore = create((set) => ({
	...
	fetchAllProducts: async ({ page = 1 } = {}) => {
		set({ loading: true });
		try {
			const response = await productApi.get(`/?page=${page}&limit=${DEFAULT_PAGE_SIZE}`);
	...
	fetchProductsByCategory: async (category, { page = 1 } = {}) => {
		...
		try {
			const response = await productApi.get(
				`/category/${category}?page=${page}&limit=${DEFAULT_PAGE_SIZE}`
			);
			const { data, total, limit, hasMore } = response.data;
			set((prevState) => ({
				products: page === 1 ? data : [...prevState.products, ...data],
	...
	toggleFeaturedProduct: async (productId) => {
		set({ loading: true });
		try {
			const response = await productApi.patch(`/${productId}`);
	...
	searchProducts: async (query, { page = 1 } = {}) => {
		...
		try {
			const response = await productApi.get(
				`/search?q=${encodeURIComponent(query.trim())}&page=${page}&limit=${DEFAULT_PAGE_SIZE}`,
